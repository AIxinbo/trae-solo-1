...
3. [Step 1：项目骨架搭建](#3-step-1项目骨架搭建)
   ...
   - [3.2 docker-compose.dev.yml](#32-docker-compose-devyml本地开发用)
   - [3.3 后端 Dockerfile](#33-后端-dockerfile)
   - [3.4 前端 Dockerfile](#34-前端-dockerfile)
   - [3.5 后端启动文件](#35-后端启动文件)
   - [3.6 SQLAlchemy 模型（13张表）](#36-sqlalchemy-模型13张表)
   - [3.7 路由注册](#37-路由注册)
   - [3.8 Pydantic Schema 示例](#38-pydantic-schema-示例)
4. [Step 2：后端核心开发](#4-step-2后端核心开发)
   - [4.1 JWT 认证中间件](#41-jwt-认证中间件)
   - [4.2 AI 客户端 + 模型配置](#42-ai-客户端--模型配置支持用户自定义模型)
   - [4.2a 模型配置 API](#42a-模型配置-api)
   - [4.3 认证路由](#43-认证路由)
   - [4.4 项目管理路由](#44-项目管理路由)
   - [4.5 大纲管理路由](#45-大纲管理路由)
   - [4.6 章节生成 API（SSE + 多智能体）](#46-章节生成-apisse-流式--多智能体管线)
   - [4.7 审查评分 API](#47-审查评分-api)
   - [4.8 细纲管理 API](#48-细纲管理-api)
   - [4.9 写作历程日志 API](#49-写作历程日志-api)
5. [Step 3：前端基础框架](#5-step-3前端基础框架)
   - [5.1 UI 设计规范](#51-ui-设计规范)
   - [5.2 页面布局结构](#52-页面布局结构)
   - [5.3 页面详细设计](#53-页面详细设计)
     - [5.3.1 首页](#531-首页)
     - [5.3.2 登录页](#532-登录页)
     - [5.3.3 注册页](#533-注册页)
     - [5.3.4 控制台](#534-控制台)
     - [5.3.5 新建项目对话框](#535-新建项目对话框)
     - [5.3.6 项目工作台](#536-项目工作台-bookid)
     - [5.3.7 大纲管理页](#537-大纲管理页-bookidoutlines)
     - [5.3.8 细纲创作页](#538-细纲创作页-bookiddetailed-outlines)
     - [5.3.9 角色管理页](#539-角色管理页-bookidcharacters)
     - [5.3.10 章节列表页](#5310-章节列表页-bookidchapters)
     - [5.3.11 写作编辑器页](#5311-写作编辑器页-bookidchapterchapterid)
     - [5.3.12 审查结果展示](#5312-审查结果展示组件)
     - [5.3.13 拆书分析页](#5313-拆书分析页-bookidanalysis)
     - [5.3.14 时间线页](#5314-时间线页-bookidtimeline)
     - [5.3.15 模型配置页](#5315-模型配置页-settingsmodels)
   - [5.4 前端 API 封装](#54-前端-api-封装)
   - [5.5 Zustand 状态管理](#55-zustand-状态管理)
6. [Step 4：AI 生成功能](#6-step-4ai-生成功能)
   - [6.1 多智能体写作管线（4 Agent）](#61-多智能体写作管线)
   - [6.2 上下文组装器（4层注入）](#62-上下文组装器)
   - [6.3 字数控制服务](#63-字数控制服务)
   - [6.4 去AI味语感评分（6维度）](#64-去ai味--语感评分)
   - [6.5 系统常量定义](#65-系统常量定义)
7. [Step 5：质量优化系统](#7-step-5质量优化系统)
   - [7.1 对话优化服务](#71-对话优化服务)
   - [7.2 时间线管理 + 角色状态追踪](#72-时间线管理)
   - [7.3 多智能体审查管线（4 Agent）](#73-多智能体审查管线)
8. [Step 6：拆书分析](#8-step-6拆书分析)
   - [8.1 拆书分析核心](#81-拆书分析核心)
9. [附录：UI 设计规范](#9-附录ui-设计规范)
   - [9.1 设计原则](#91-设计原则)
   - [9.2 全局状态样式](#92-全局状态样式)
   - [9.3 响应式断点](#93-响应式断点)
   - [9.4 动画规范](#94-动画规范)
   - [9.5 组件样式速查](#95-组件样式速查)
   - [9.6 常用图标清单](#96-常用图标清单)
...
#### 5.3.11 写作编辑器页 (`/book/[id]/chapter/[chapterId]`)
...
Agent 2 (Writer)：按分镜逐段生成正文 → 流式输出给用户
   └─ system prompt 已内置完整去AI味写作规范 ← 前置去AI规则
     ↓
Agent 3 (DeAI)：质量门禁 → 语感评分 ≥75 则合格通过
   └─ 仅当分数 < 75 才触发改写 ← 后置检查兜底
     ↓
...
**关键设计原则**：去AI味规则前置到 Writer Agent 的 system prompt 中，让 AI 在写作时就遵循规范；DeAI Agent 仅作质量门禁检查，绝大多数情况下无需改写，避免浪费 API 调用。
...
1. **字数控制**：严格按照场景的目标字数写作，不超不缩

2. **角色一致性**：
   - 角色行为、语言、决策必须符合其性格设定
   - 对话要有角色区分度（不同角色说话方式不同、用词不同）
   - 使用角色设定中的爱用词，避免禁用词
   - 角色知识边界：角色不知道的信息不要出现在其视角中

3. **去AI味（写作时必须严格遵守）**：
   ❌ 禁止使用以下AI高频词：
     然而、但是、不过、值得一提的是、显而易见、毫无疑问、
     事实上、换句话说、简而言之、不可否认、值得注意的是、
     总体而言、从某种角度来说、在一定程度上、需要指出的是
   ...
   ❌ 禁止过于工整的排比句和过度完整的句式
     错误示例：他既感到愤怒，又感到无奈，还感到一丝悲哀
     正确示例：他气得想骂人，可话到嘴边又咽了回去

   ✅ 句子长短交替，至少30%的短句（10字以内）
   ✅ 对话占比不低于40%，多用语气词（啊、嘛、吧、呢、啦、呗）
   ✅ 段落短小，每段不超过200字
   ✅ 句子开头要多样化，不要总是"他/她/这/那"
   ✅ 用动作展示情绪，不要直接告诉读者角色的感受

4. **网文节奏**：
   - 每1500-2000字设立一个爽点（打脸/逆袭/收获/反转/揭露）
   - 紧张场景与轻松场景交替出现
   - 关键情节展开写，过渡情节快写、不拖沓"""
...
"""
去AI味 Agent — 质量门禁，非主要改写工具
位于 generator/ 下，作为多智能体写作管线的 Agent 3

设计原则：
  Writer Agent 的 system prompt 已内置完整去AI味写作规范，
  DeAI Agent 仅作后置质量门禁检查：
    ① 对 Writer 输出的正文进行6维度语感评分
    ② 评分 ≥75 直接通过，不做任何修改
    ③ 评分 <75 才触发 AI 改写Pass，改写后重新评分
  绝大多数情况下（>90%）不会触发改写，避免浪费 API 调用。
"""
