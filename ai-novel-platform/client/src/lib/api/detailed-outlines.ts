...
export const detailedOutlinesApi = {
  ...
  get: (id: string) => api.get<DetailedOutline>(`/books/detailed-outlines/${id}`),
  update: (id: string, data: Partial<DetailedOutline>) => api.put<DetailedOutline>(`/books/detailed-outlines/${id}`, data),
  delete: (id: string) => api.delete<void>(`/books/detailed-outlines/${id}`),
